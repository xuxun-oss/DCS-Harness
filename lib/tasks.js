...
export function upsertTask(input, sessionId) {
...
export function updateStepStatus(taskId, stepId, status, detail, progress, sessionId) {
  const tasks = loadAllTasks();
...
export function mergeTask(taskId, patch, sessionId) {
  const tasks = loadAllTasks();
  const task = tasks.find((t) => t.id === taskId && (!sessionId || t.sessionId === sessionId));
  if (!task) return { ok: false, error: '任务不存在: ' + taskId };
