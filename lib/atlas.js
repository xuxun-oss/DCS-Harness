...
export const GENPILOT_PATTERN = {
  ...
  note: 'LLM_API_KEY 与 dcs_pat（CLI 认证）、GENOS_API_KEY（Genos 模型）三者互不相同。',
};

export function genpilotHints() {
  return `Genpilot 智能分析典型用法（实测 PGP）：
1. 在在线容器 /work/{user}/{project}_{date}/ 建项目目录，写 plan.md（步骤进度表）。
2. 写 scripts/stepN_*.py 分步脚本；私有密钥放 .env（set -a && source .env && set +a 加载）。
...
5. 解读与写作：dcs_llm 调 DCS 托管 Genpilot LLM（deepseek-v4-pro），用于文献综合/变异解读/论文写作。
