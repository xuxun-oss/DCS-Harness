...
`;
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      steps: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { id: { type: 'string', required: true }, title: { type: 'string', required: true }, status: { type: 'string', enum: ['pending', 'running', 'done', 'failed', 'blocked'] }, detail: { type: 'string' }, dependsOn: { type: 'array', items: { type: 'string' } }, outputs: { type: 'array', items: { type: 'string' } }, progress: { type: 'integer' } } }, description: '分析步骤列表 [{id, title, status, detail, dependsOn(依赖步骤id=逻辑关系), outputs, progress(0-100)}]' },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
