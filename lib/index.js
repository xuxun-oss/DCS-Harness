...
`;
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      try {
        const r = upsertTask(args, sessionIdOf(exec));
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      const r = updateStepStatus(str(args.task_id), str(args.step_id), args.status, args.detail, args.progress, sessionIdOf(exec));
      ...
      try {
        ...
        if (req.method === 'POST' && path === '/tasks') {
          readBody(req).then((body) => {
            ...
            try {
              const r = upsertTask(body, body.sessionId || sessionId);
        ...
        if ((req.method === 'PATCH' || req.method === 'POST') && path.startsWith('/tasks/')) {
          ...
          if (!body) { json(400, { ok: false, error: '无效 JSON' }); return; }
          const r = mergeTask(id, body, body.sessionId || sessionId);
...
2. 找数据（容器 /public 优先）：先用 dcs_container_ls 列 /public（公共库挂载）与 /public/database/CNGBdb/pub/SciRAID/stomics/（STDS 编号数据集）定位；用 dcs_data_inspect 看 h5ad 结构（细胞类型/基因）；容器 /public 里没有时再用 dcs_public_search 搜公共库元数据；避免重复：先 dcs_find_results 看 /work 里 Genpilot 是否已跑过。
...
5. 数据资源（第一优先级）：dcs_container_ls 列容器 /public 公共数据 → 其次 dcs_public_search 搜公共库、dcs_data_find 查 /Files；不足时（第二优先级）dcs_data_download 外部下载。
