...
import {
  loadCfg, saveCfg, runDcs, dcsLogin, dcsStatus, dcsLlm,
  resolveDcsBinary, platformBinary, DCS_VERSION,
} from './dcs-client.js';
import { auditScript } from './audit.js';
import { generateReport } from './report.js';
import { generatePlan } from './plan.js';
import { REGIONS, OMICS_TOOLS, KEYWORD_TO_CATEGORY, searchHints, genpilotHints, GENPILOT_PATTERN } from './atlas.js';
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_configure',
    description: '配置 dcs CLI 与 Genpilot LLM 的本地参数（cliPath、autoInstall、llmApiBase/llmApiKey/llmModel）。PAT 登录请用 dcs_login。',
    parameters: {
      ...
      llmApiBase: { type: 'string', description: 'Genpilot LLM API 端点（OpenAI 兼容），默认 https://dcsapi.dcs.cloud/api/aigress/unified/v1/chat/completions' },
      llmApiKey: { type: 'string', description: 'Genpilot LLM API Key（与 dcs_pat 不同，取自容器 .env 的 LLM_API_KEY 或 DCS 平台申请）' },
      llmModel: { type: 'string', description: 'Genpilot LLM 模型名，默认 deepseek-v4-pro' },
    },
    output: {
      schema: {
        ...
        properties: {
          ...
          llmConfigured: { type: 'boolean', required: true },
          error: { type: 'string' },
        },
      },
      render(args, v) { return [{ type: 'text', text: v.ok ? ('✅ dcs 配置已保存：cliPath=' + v.cliPath + '，autoInstall=' + v.autoInstall + '，Genpilot LLM ' + (v.llmConfigured ? '已配置' : '未配置') + '（平台 ' + v.platform + '）') : ('❌ 配置失败: ' + (v.error || '')) }]; },
    },
    async execute(args) {
      const cfg = loadCfg();
      const next = { ...cfg };
      ...
      if (str(args.llmApiBase)) next.llmApiBase = str(args.llmApiBase);
      if (str(args.llmApiKey)) next.llmApiKey = str(args.llmApiKey);
      if (str(args.llmModel)) next.llmModel = str(args.llmModel);
      saveCfg(next);
      return { ok: true, cliPath: next.cliPath, autoInstall: next.autoInstall, platform: platformBinary() || (process.platform + '/' + process.arch), llmConfigured: !!next.llmApiKey, error: '' };
    },
  }));
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      section: { type: 'string', enum: ['all', 'regions', 'tools', 'keywords', 'hints'], description: '返回哪部分：all=全部（默认），regions=片区公共库，tools=官方工具库，keywords=关键词映射，hints=检索与 Genpilot 使用范式' },
    },
    output: {
  ...
  ctx.tools.register(defineTool({
    name: 'dcs_llm',
    description: '调用 DCS 托管的 Genpilot LLM（OpenAI 兼容，deepseek-v4-pro 等）做变异解读、文献综合、方案撰写、论文润色等。需先在 dcs_configure 里配 llmApiKey（与 dcs_pat 不同，取自容器 .env 的 LLM_API_KEY）。',
    parameters: {
      prompt: { type: 'string', required: true, description: '用户提问/要完成的任务' },
      system: { type: 'string', description: '系统提示词（角色设定），可选' },
      model: { type: 'string', description: '模型名，默认取配置的 llmModel（deepseek-v4-pro）' },
      temperature: { type: 'number', description: '采样温度 0~1，默认不设' },
    ...
    output: {
      ...
      render(args, v) { return [{ type: 'text', text: v.ok ? ('🤖 Genpilot LLM（' + v.model + '）：\n' + v.content) : ('❌ 失败: ' + (v.error || '')) }]; },
    },
    timeoutMs: 300000,
    async execute(args, exec) {
      const cfg = loadCfg();
      try {
        const messages = [];
        if (str(args.system)) messages.push({ role: 'system', content: str(args.system) });
        messages.push({ role: 'user', content: str(args.prompt) });
        const r = await dcsLlm(cfg, { messages, model: str(args.model), temperature: args.temperature, max_tokens: args.max_tokens, timeoutMs: 180000 }, exec && exec.signal);
        return { ok: true, model: r.model, content: r.content, error: '' };
      } catch (e) {
...
1. 定方向：用 dcs_atlas 查「数据库全图谱」（片区公共库 + 官方组学工具库 + 关键词映射），确定该去哪个片区、用哪个官方流程找数据。
2. 任务分解：把任务拆成可并行的子任务；用 dcs_plan 生成 Plan.md（步骤进度表）追踪，可用 dcs_workflow_info 的 plan 做多步规划。
3. 深度研究：用 web_search / 子代理检索文献与方法；需求不明时用 ask_user_question 与客户确认，形成研究方案（目标/方法/数据/脚本/执行计划）。
4. 数据资源（第一优先级）：用 dcs_data_find / dcs_data_ls 在 DCS 平台数据（/Files 公共库与项目数据）检索；不足时（第二优先级）用 dcs_data_download 获取外部数据。
5. 脚本/方案（第一优先级）：用 dcs_workflow_search / dcs_workflow_info 复用 Genpilot 现有流程与参数；不足时（第二优先级）用 dcs_terminal_file 编写新脚本（分步 stepN_*.py，密钥放 .env）。
6. 审计：执行前用 dcs_audit_script 审计所有脚本，确认无 critical/high 风险。
7. 执行：耗时久/资源大/需并行 → dcs_offline_run / dcs_parallel_run（{i} 分片并行投递）或 dcs_workflow_run；简单任务 → dcs_terminal_exec 在线运行。标准镜像 ubuntu:24.04-python3.12、资源 4c 16g。用 dcs_task_status 跟踪。
8. 解读与写作：用 dcs_llm 调 DCS Genpilot LLM（deepseek-v4-pro）做变异解读/文献综合/论文写作。
9. 报告：用 dcs_generate_report 出 HTML 网页、dcs_plan 更新 Plan.md，把结果/图表/方法/思路按学术逻辑交付。
