...
export const DEFAULT_CFG = {
  ...
  autoInstall: 'auto',
  // Genpilot LLM API（DCS 托管，OpenAI 兼容）。LLM_API_KEY 与 dcs_pat / GENOS_API_KEY 不同。
...
// ---- Genpilot LLM API（DCS 托管，OpenAI 兼容）----

/**
 * 调用 DCS Genpilot 托管 LLM（dcsapi.dcs.cloud 的 /chat/completions）。
 * 用于变异解读、文献综合、论文写作等（genpilot 能力）。需配置 llmApiKey。
 */
export async function dcsLlm(cfg, args, signal) {
  const base = (cfg.llmApiBase || DEFAULT_CFG.llmApiBase).trim();
  const key = cfg.llmApiKey || process.env.DCS_LLM_API_KEY || '';
  if (!key) throw new Error('未配置 Genpilot LLM API Key（llmApiKey）。请在 dcs_configure 里设置，或在容器 .env 读取 LLM_API_KEY。');
  const messages = args.messages || [{ role: 'user', content: args.prompt }];
  const body = {
    model: args.model || cfg.llmModel || DEFAULT_CFG.llmModel,
    messages,
  };
  ...
  const t = AbortSignal.timeout(args.timeoutMs || 180000);
  const sig = signal ? AbortSignal.any([signal, t]) : t;
  ...
  if (!res.ok) {
    ...
    throw new Error('Genpilot LLM 调用失败 HTTP ' + res.status + ': ' + msg);
  }
  // 提取文本
  let content = '';
  try { content = data.choices[0].message.content; } catch { content = text; }
  return { model: data.model || body.model, content, raw: data };
}
